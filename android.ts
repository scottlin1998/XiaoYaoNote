...
import { dirRecursion, getDaysBetween, getDay, createDirs, toTrashBin, createDeepDir, getFileMd5 } from "./others/utils";
        ...
        switch (operator) {
            ...
            case "@":
                ...
                if (id && (!note.existData() || !note.existFile() || !note.canReview())) toTrashBin(filePath);
                    ...
                    note.levelUp();
                    // #兼容安卓#
                    let linkMd5: string = getFileMd5(filePath);
                    let noteMd5: string = getFileMd5(notePath);
                    // 如果 复习链接 和 笔记 的md5不相同，则替换源文件
                    if (linkMd5 != noteMd5) fs.renameSync(filePath, notePath); else toTrashBin(filePath);
                } else if (!id) {
